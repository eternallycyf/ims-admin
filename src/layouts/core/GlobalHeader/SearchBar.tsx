...
export default function SearchBar() {
  ...
  useKeyPress('downarrow', (event) => {
    ...
    event.preventDefault();
    let nextIndex = selectedItemIndex + 1;
    if (nextIndex > searchResult.length - 1) nextIndex = 0;
  ...
  return (
    <>
      <div className="flex items-center justify-center">
        <IconButton
          className="bg-hover h-8 rounded-xl py-2 text-xs font-bold"
          onClick={handleOpen}
        >
          <div className="flex items-center justify-center gap-2">
            <SvgIcon icon="ic-search" size="20" />
            <span className="h-6 flex items-center justify-center rounded-md bg-[#fff] px-1.5 text-gray-800 font-bold">
              {' '}
              ⌘K{' '}
            </span>
          </div>
            ...
            <div ref={listRef} className="py-2">
              {searchResult.map(({ key, label }, index) => {
                const partsTitle = parse(
                  t(label as any),
                  match(t(label as any), searchQuery),
                );
